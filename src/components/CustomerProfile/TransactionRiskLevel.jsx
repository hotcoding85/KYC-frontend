...
export const TransactionRiskLevel = () => {
  return (
    <div className="w-full p-4 mt-4 bg-white rounded-xl md:mt-0">
      <h2 className="mb-3 text-[14px] font-semibold leading-[20px] tracking-[-0.005em] text-left mb-[20px] text-textBlack">Transaction Risk Level</h2>
      <div className="grid gap-4 md:grid-cols-2">
        <div className="flex justify-between">
          ...
          <h3 className="font-semibold text-xs leading-[16px] text-right text-textSecondary">1,200 BTC</h3>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3 mt-3 text-xs">
          {transactionData.map((item, index) => (
            <div key={index} className="flex justify-between">
              ...
              <span className="font-semibold text-[12px] leading-[16px] text-right text-textSecondary">{item.value}</span>
            </div>
          ))}
        </div>
        <div className="flex flex-col gap-3 mt-3 text-xs">
