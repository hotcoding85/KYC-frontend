        ...
        if (option === "View") {
            return (
                ...
                <div className="flex items-center">
                    ...
                    <span className={`ml-4 px-4 py-1 rounded-full text-sm ${isActive ? "bg-successLight text-success" : "bg-alertLight text-red-600"}`}>
            ...
            return (
                <div className="flex items-center">
                    {(option === "Add" || option === "Edit") && <span classNmae="cursor-pointer" onClick={goBack}>
                        {<Back />}
                    </span>}
                    <h2 className="text-sm font-medium text-textBlack ml-2">
                        {option === "Add" ? "Add Team Member" : option === 'Edit' ? "Edit Team Member" : ''}
                    </h2>
                </div>
            );
        }
    ...
    const renderRightSection = () => {
        return (
            <div className="flex items-center space-x-4">
                ...
                {!isAdd && (
                    <button onClick={handleOption} className={`px-5 h-8 text-center text-sm py-2 border flex items-center justify-center rounded-xl ${isActive ? "border-red-600 text-red-600" : "border-green-600 text-success"}`}>
