...
const AssetsSweepScheduleDataTableComponent = ({
  data,
  title,
  btnText,
  isSortable,
      ...
      {results ? (
        ...
        >
          ...
          <Column
            ...
            body={(rowData) => {
              return <div>
                <p className="cursor-pointer line-clamp-1">{rowData.scheduleOn}</p>
              </div>
            }
            }
          ...
          <Column
            ...
            body={(rowData) => {
              ...
              return (
                rowData.scheduleOn === 'DURATION' ? 
                <p className="text-xs text-gray-500">
                  {rowData.repeat} {rowData.duration}
                </p>
                :
                <div>
                  {rowData.rules && rowData.rules.length > 0 ? (
                    rowData.rules.map((rule, index) => (
                      <p key={index} className="text-xs text-gray-500">
                        {comparasionMap[rule.comparasion] || rule.comparasion} {rule.value}
                      </p>
                    ))
                  ) : (
                    <p className="text-xs text-gray-500">No rules</p>
                  )}
                </div>
              );
            }}
...
export default AssetsSweepScheduleDataTableComponent;
