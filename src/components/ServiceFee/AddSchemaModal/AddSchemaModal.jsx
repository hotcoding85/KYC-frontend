...
import DynamicFeeCriteria from "@/components/Companies/ServiceFeeTab/DynamicFeeCriteria";
import {
  FEE_SCHEME_TYPE,
  FEE_SCHEME_FEE_TYPE,
...
const AddSchemeModal = ({
    ...
    if (validationResult.success) {
      ...
      if (result) {
        ...
        toast.current.show({
          severity: "success",
          summary: "Saved",
          detail: "Company Fee Scheme had been saved successfully!",
          life: 3000,
        });
      } else {
        setLoading(false);
  ...
  const fetchAssets = async () => {
    ...
    if (error) {
      ...
      TypeEnum.current = z.enum([...result.map(asset => {return asset.name})])
      console.log(TypeEnum.current)
      assetSchema.current = z.object({
        feeType: feeTypeEnum,
        assetType: TypeEnum.current,
                ...
                defaultValue={
                  formData && formData.assetType
                    ? {
                        ...
                        label: FEE_SCHEME_TYPE[formData.assetType],
                      }
                    : undefined
                }
                items={assetTypes}
